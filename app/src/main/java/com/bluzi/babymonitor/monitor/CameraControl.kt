...
object CameraControl {
    ...
    suspend fun getNightVision(store: AppStore): NightVisionMode? = withContext(Dispatchers.IO) {
    ...
    suspend fun setNightVision(store: AppStore, mode: NightVisionMode) = withContext(Dispatchers.IO) {
        val (cloud, did) = cloud(store)
