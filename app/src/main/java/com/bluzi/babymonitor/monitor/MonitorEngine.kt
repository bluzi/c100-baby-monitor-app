...
import com.bluzi.babymonitor.net.JavaSocketFactory
 ...
 * Owned by MonitorService; state surfaces through MonitorHub.
 */
class MonitorEngine(
    ...
    @Volatile
    private var player: OpusAudioPlayer? = null
    ...
    fun start() {
        ...
        auxJobs += scope.launch {
            while (isActive) {
                ...
                val minutesOfDay = LocalTime.now().let { it.hour * 60 + it.minute }
                watchdog.enabled = StreamWatchdog.armed(s.watchdogEnabled, s.alarmEnabled, schedule, minutesOfDay)
    ...
    private suspend fun connectOnce() {
        ...
        val missClient = MissClient(device.model, JavaSocketFactory)
        client = missClient
        ...
        try {
            ...
            val audioPlayer = OpusAudioPlayer(onPcmWindow = ::onPcmWindow)
            ...
            // per retry would exhaust the phone's decoders — and a phone with no decoder left
            // has no audio, no level meter and no noise alarm.
            player = audioPlayer
            audioPlayer.muted = MonitorHub.settings.value.muted
            audioPlayer.start()
            // HevcRenderer.push never throws — video trouble never takes audio down (LIVE-7).
            val videoRenderer = HevcRenderer()
            ...
            coroutineScope {
                ...
                launch {
                    ...
                    for (f in videoQueue) {
                        val backlog = videoBacklog.decrementAndGet()
                        if (catchup.admit(Hevc.isKeyframe(f.data), backlog)) {
                            videoRenderer.push(f.data, f.pts, MonitorHub.surface.value)
                        }
                    }
                }
                    ...
                    while (isActive && MonitorHub.running.value) {
                        ...
                        when (frame) {
                            is Frame.Audio -> if (frame.codec == "opus") {
                                ...
                                val seen = unsupportedAudio.incrementAndGet()
                                if (seen == 1) {
    ...
    private fun logRoomLevel(levelDb: Double, nowMs: Long) {
        ...
        Log.d(
            "engine",
            "room level last %ds: median %.1f max %.1f dB above ambient (floor %.1f dBFS, %d windows)".format(
                java.util.Locale.ROOT,
                (nowMs - levelStatsStartMs) / 1000,
    ...
    private fun onPcmWindow(pcm: ShortArray, sampleRate: Int) {
        ...
        if (detector.onWindow(level, metrics, windowMs, now)) {
            ...
            val minutesOfDay = LocalTime.now().let { it.hour * 60 + it.minute }
            if (schedule.isActive(minutesOfDay)) {
