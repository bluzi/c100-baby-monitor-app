...
class LevelMeterTest {
    ...
    fun `LIVE-6 a loud onset over a quiet room reads immediately`() {
        ...
        assertTrue("expected a strong level, got $level", level > 10.0)
    }
    ...
    fun `LIVE-6 constant background noise adapts back toward quiet`() {
        ...
        assertTrue("initially audible ($initial)", initial >= 0.0)
        assertTrue("should settle near the floor, got $settled", settled < 2.0)
    }
    ...
    fun `LIVE-6 an event louder than the settled background still stands out`() {
        ...
        assertTrue("cry should stand out over settled fan, got $level", level > 8.0)
    }
    ...
    fun `LIVE-6 a fluttering background reads flat — and never loud enough to alarm`() {
        ...
        assertTrue("a steady room should read ~0, got median $median", median <= 0.5)
        ...
        assertTrue("flutter reached $bar dB (max was ${sorted.last()})", sorted.last() < bar)
    }
    ...
    fun `LIVE-6 a cry onset over a fluttering background still reads immediately`() {
        ...
        assertTrue("a cry over flutter should stand out, got $level", level > 10.0)
    }
    ...
    fun `LIVE-6 ongoing crying with breaths between bursts is never absorbed into the baseline`() {
        ...
        assertTrue("after minutes of crying, bursts must still stand out, got $lastBurst", lastBurst > 8.0)
    }
}
