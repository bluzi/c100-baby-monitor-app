...
/**
 * Shared state between the monitor (which owns itself — BG-5) and whatever UI happens to be
 * attached. The UI is a thin observer; the monitor is the owner. On Android the monitor lives in
 * a foreground service, on macOS in the tray-resident process; neither difference reaches here.
 *
 * There is deliberately no video surface here. Where the picture goes is the one thing that is
 * purely a platform's business (a Surface, a CALayer), and holding it in shared state was what
 * used to drag `android.view.Surface` into the engine. Each platform's VideoOutput owns its own
 * output target now.
 */
object MonitorHub {
    val running = MutableStateFlow(false)
    val status = MutableStateFlow(STATUS_IDLE)
    ...
    /**
     * Monotonic timestamp of the last *decodable audio* frame — feeds the watchdog's liveness
     * check (WATCH-2). Audio is what monitoring means: video alone, or audio in a codec we cannot
     * play, is not a live feed (WATCH-7). Wall clock would let an overnight NTP/DST correction
     * hide an outage, so it must never be used here.
     */
    @Volatile
    var lastAudioAtMs: Long = 0L

    /** Set by the engine; routes acknowledge presses (app button, notification, tray menu). */
    @Volatile
