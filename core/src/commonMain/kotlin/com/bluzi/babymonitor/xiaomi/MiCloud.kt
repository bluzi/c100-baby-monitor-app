...
import com.bluzi.babymonitor.json.JSONArray
...
/** Xiaomi's "wrong account or password" (AUTH-9). */
private const val WRONG_CREDENTIALS = 70016
private const val NO_CODE = Int.MIN_VALUE
...
fun buildSignedForm(path: String, data: String, ssecurity: ByteArray, nonce: ByteArray): Pair<String, ByteArray> {
    ...
    val encData = Crypto.rc4(signedNonce, data.encodeToByteArray()).toBase64()
    val encHash = Crypto.rc4(signedNonce, rc4Hash.encodeToByteArray()).toBase64()
...
class MiCloud(
    ...
    private fun loginFailureMessage(v2: JSONObject): String {
        val code = if (v2.isNull("code")) null else v2.optInt("code", NO_CODE).takeIf { it != NO_CODE }
        if (code == WRONG_CREDENTIALS) return "Wrong account or password."
    ...
    private suspend fun doRequest(apiPath: String, params: String): Any {
        ...
        if (res.status != 200) {
            Log.w("cloud", "$apiPath HTTP ${res.status}: ${res.body.decodeToString().take(200)}")
            throw XiaomiException("xiaomi: ${res.status} on $apiPath — ${res.body.decodeToString().take(300)}")
        }
        val decoded = Crypto.rc4(signedNonce, res.body.decodeToString().base64ToBytes())
        val json = JSONObject(decoded.decodeToString())
