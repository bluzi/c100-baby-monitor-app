...
class AlarmTonesTest {
    ...
    fun `ALRM-11 every alarm sound actually makes a sound — and none of them clips`() {
        for (sound in Settings.ALARM_SOUNDS) {
            ...
            assertTrue(peakOf(pcm) > 0.3, "$sound is silent")
            assertTrue(peakOf(pcm) <= 1.0, "$sound would clip")
            // Long enough to be heard, short enough to repeat: repetition is what wakes people.
            assertTrue(seconds in 1.0..4.0, "$sound cycle is ${seconds}s")
        }
    }
    ...
    fun `ALRM-11 every alarm sound ends in a gap — so it repeats instead of droning`() {
        for (sound in Settings.ALARM_SOUNDS) {
            ...
            assertTrue(tailPeak < 0.1, "$sound does not fall quiet before it loops (tail peak $tailPeak)")
        }
    }
    ...
    fun `ALRM-11 the alarms are told apart by ear — calm and urgent do not converge on one pitch`() {
        ...
        assertTrue(lowPulse < 350, "the low pulse must actually be low ($lowPulse Hz)")
        assertTrue(urgentBeep > 700, "the urgent beep must be high ($urgentBeep Hz)")
        assertTrue(softChime in 350.0..1500.0, "the soft chime sits between them ($softChime Hz)")
    }
    ...
    fun `ALRM-14 the alarm starts gentler and reaches full volume within a few seconds`() {
        assertTrue(alarmRampGain(0) >= 0.3, "it must never start silent")
