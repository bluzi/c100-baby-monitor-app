...
class CryCalibrationTest {
    ...
    fun `ALRM-2 higher sensitivity means quieter sound can trigger`() {
        for (s in Settings.SENSITIVITY_MIN until Settings.SENSITIVITY_MAX) {
            assertTrue(sensitivityThresholdDb(s + 1) < sensitivityThresholdDb(s), "sensitivity ${s + 1} must demand less loudness than $s")
        }
    }
    ...
    fun `ALRM-16 a false-alarm answer makes triggering one step harder`() {
        ...
        assertTrue(after > before, "a false alarm must raise the bar")
    ...
    fun `ALRM-16 learning is bounded and never goes below the slider setting`() {
        ...
        assertEquals(sensitivityThresholdDb(Settings.SENSITIVITY_DEFAULT), effectiveThresholdDb(Settings.SENSITIVITY_DEFAULT, 0), 1e-9)
    }

    @Test
    fun `ALRM-16 corrupt stored steps are treated as their nearest bound`() {
        assertEquals(effectiveThresholdDb(Settings.SENSITIVITY_DEFAULT, 0), effectiveThresholdDb(Settings.SENSITIVITY_DEFAULT, -7), 1e-9)
        assertEquals(effectiveThresholdDb(Settings.SENSITIVITY_DEFAULT, CALIBRATION_MAX_STEPS), effectiveThresholdDb(Settings.SENSITIVITY_DEFAULT, 99), 1e-9)
    }
