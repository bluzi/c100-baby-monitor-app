...
class CryDetectionTest {
    ...
    fun `ALRM-3 crying triggers across the whole range of infant pitches`() {
        for (f0 in listOf(300.0, 400.0, 500.0, 600.0)) {
            ...
            assertTrue(fired, "a cry at ${f0}Hz must trigger the alarm")
        }
    }
    ...
    fun `ALRM-5 after acknowledgment the alarm stays quiet for the cooldown — then may trigger again`() {
        ...
        assertFalse(cryFor(25.0), "crying inside the cooldown must not re-alarm")
        assertTrue(cryFor(10.0), "crying after the cooldown must alarm again")
    }
}
