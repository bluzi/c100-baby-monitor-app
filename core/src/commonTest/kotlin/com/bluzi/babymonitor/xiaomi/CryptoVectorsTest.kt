 ...
 * (core/protocol-vectors.json, compiled into the test binary by the build — Kotlin/Native has no
 * classpath resources). If these pass, our bytes match the implementation that actually talks to
 * the camera. And because this class runs on the JVM *and* on macOS, it proves it for both apps:
 * the phone and the Mac cannot drift apart on the wire.
 */
class CryptoVectorsTest {
    private val v: JSONObject by lazy { JSONObject(PROTOCOL_VECTORS_JSON) }
    ...
    fun `PROTO-8 rc4 discards 1024 keystream bytes and matches the reference`() {
        ...
        val out = Crypto.rc4(rc4.getString("keyHex").hexToBytes(), rc4.getString("plaintextUtf8").encodeToByteArray())
    ...
    fun `PROTO-8 rc4 is symmetric`() {
        ...
        val plain = "round trip".encodeToByteArray()
        assertContentEquals(plain, Crypto.rc4(key, Crypto.rc4(key, plain)))
    }
    ...
    fun `PROTO-14 chacha20 with 8-byte nonce padded to 12 matches the reference`() {
        ...
        val out = Crypto.chachaDecodeNonce(
            c.getString("plaintextUtf8").encodeToByteArray(),
    ...
    fun `PROTO-14 encode prepends fresh nonce and decode round-trips`() {
        ...
        val plain = "wire round trip".encodeToByteArray()
        ...
        assertContentEquals(plain, Crypto.chachaDecode(enc1, key))
        assertContentEquals(plain, Crypto.chachaDecode(enc2, key))
    }
    ...
    fun `PROTO-8 response bodies decrypt with the signed nonce`() {
        ...
        assertEquals(r.getString("plaintext"), plain.decodeToString())
    }
}
