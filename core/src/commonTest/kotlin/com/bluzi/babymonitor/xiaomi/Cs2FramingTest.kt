...
class Cs2FramingTest {
    ...
    fun `PROTO-17 command frames carry BE sizes — channel — seq — and LE-free layout`() {
        ...
        assertContentEquals(payload, frame.copyOfRange(16, frame.size))
    }
    ...
    fun `PROTO-16 tcp frames have BE u16 size and 0x68 magic in an 8-byte header`() {
        ...
        assertContentEquals(body, framed.copyOfRange(8, framed.size))
    }

    @Test
    fun `PROTO-17 udp acks echo channel and sequence`() {
        val ack = Cs2.udpAck(2, 0x01, 0x02)
        assertContentEquals(byteArrayOf(0xf1.toByte(), 0xd1.toByte(), 0, 6, 0xd1.toByte(), 2, 0, 1, 0x01, 0x02), ack)
    }

    @Test
    fun `PROTO-18 records reassemble regardless of chunk boundaries`() {
        val asm = RecordAssembler()
        val record = "hello miss".encodeToByteArray()
        ...
        assertContentEquals(record, out[0])
    }
    ...
    fun `PROTO-18 a corrupt record length is a dead connection — not a crash`() {
        ...
        val err = assertFailsWith<XiaomiException> { asm.push(corrupt) }
        ...
        assertFailsWith<XiaomiException> { RecordAssembler().push(huge) }
    }
    ...
    fun `PROTO-18 multiple records in one chunk all come out`() {
        ...
        assertContentEquals(recA, out[0])
        assertContentEquals(recB, out[1])
    }
}
