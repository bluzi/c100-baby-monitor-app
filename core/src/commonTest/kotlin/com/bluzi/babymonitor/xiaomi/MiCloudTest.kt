...
private fun loginBody(json: String) = "&&&START&&&$json".encodeToByteArray()
...
class MiCloudTest {
    ...
    fun `PROTO-2 a login response without the START prefix is an error`() = runTest {
        val http = FakeMiHttp()
        http.handler = { resp(body = "<html>maintenance</html>".encodeToByteArray()) }
        val err = assertFailsWith<XiaomiException> {
            MiCloud(http).login("u", "p")
        }
    ...
    private fun encryptResponse(req: FakeMiHttp.Req, ssecurity: ByteArray, plaintext: String): ByteArray {
        ...
        return Crypto.rc4(signedNonce, plaintext.encodeToByteArray()).toBase64().encodeToByteArray()
    }
        ...
        http.handler = { req ->
            ...
            when {
                url.endsWith("/v2/home/device_list_page") -> {
                    apiCalls++
                    if (apiCalls == 1) {
                        resp(status = 401, body = "Unauthorized".encodeToByteArray())
                    } else {
    ...
    fun `PROTO-5+AUTH-8 a refresh whose redirect chain dies part-way stays retryable and mixes no tokens`() = runTest {
        ...
        assertFalse(err is AuthExpiredException, "a half-finished refresh must stay retryable")
        assertNull(refreshed, "a mixed session must never be persisted")
        assertEquals(1, apiCalls, "the request must not be retried with mixed tokens")
    }

    @Test
    fun `APP-3 non-auth server failures surface path and status`() = runTest {
        val http = FakeMiHttp()
        http.handler = { resp(status = 500, body = "boom".encodeToByteArray()) }
        val cloud = MiCloud(http, session = session())
        val err = assertFailsWith<XiaomiException> {
            cloud.request("/v2/home/device_list_page", "{}")
        }
    ...
    fun `PROTO-24 miotGetProp reads a property value`() = runTest {
        ...
        http.handler = { req ->
            ...
            val signedNonce = Crypto.genSignedNonce(ssecurity, nonce)
            val data = JSONObject(Crypto.rc4(signedNonce, form["data"]!!.base64ToBytes()).decodeToString())
