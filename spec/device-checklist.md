...
D4h. **Locking the mini tile for a game (DESK-28), on a Mac and a PC:** put the window in its mini
     shape over a full-screen game (a borderless-windowed FPS is the case this exists for; a Mac's own
     full-screen game hides the menu bar, so the tile still floats over it but reach the menu-bar item
     by pushing to the top of the screen). Open the status-area menu — the menu-bar item on a Mac, the
     tray icon on a PC — and tick **Lock Mini Window (click-through)**; the item appears only while the
     tile is the shape on screen, never in the full window. Now:
     ...
     - **Trigger an alarm** (a sharp noise, feed muted or not): the tile **stays faint and
       ...
       **sounds** at its set volume, and the **status icon turns to the alarm mark** (DESK-1). **Click
...
D4i. **Offline is said out loud (LIVE-13):** turn every network interface off (Wi-Fi *and*
