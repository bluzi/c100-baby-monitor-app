...
- **DESK-28** `[desktop]` **The mini tile can be locked out of the way, so it floats over a full-screen
  game without ever interrupting it.** The lock is offered in the status-area menu — the menu-bar item's
  menu on a Mac, the tray menu on a PC — and only while the tile is the shape on screen; the full window
  is never affected by it. While the tile is locked:
  ...
  - **it stays locked even while an alarm rings.** This is the one place the tile does not obey
    ...
    status icon acknowledges a ringing alarm** (the menu-bar item on a Mac, the tray icon on a PC; a
    right-click — or a Control-click on a Mac — still opens the menu). This is the fastest way to silence
    an alarm on a desktop, and the only one that needs neither the tile nor the menu.
