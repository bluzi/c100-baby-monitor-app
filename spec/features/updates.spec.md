...
- **UPD-3** `[macos]` `[windows]` `[device]` A desktop app updates itself, and it does so **only at
...
- **UPD-5** `[macos]` `[windows]` `[device]` **The app never restarts itself.** A monitor that
...
- **UPD-5w** `[windows]` `[device]` Windows will not let a running program overwrite its own files,
  so on a PC the verified update waits *beside* the app rather than on top of it, and the swap
  happens at the next launch, before monitoring starts. The bargain a parent is offered is exactly
  UPD-5's — never a restart they did not ask for, one question, and declining costs nothing.
- **UPD-7** `[macos]` `[windows]` `[device]` The version an update put in place is visible before it
...
- **UPD-9** `[macos]` `[windows]` `[device]` A check can always be asked for by hand — from the menu
